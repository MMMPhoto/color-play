...
// Render Board based on Window Size
  ...
  useEffect(() => {
    if (freshClick) {
      ...
      const adjacentSquares = [(freshClick - boardWidth), (freshClick + boardWidth)];
      if (freshClick % boardWidth !== 0) {adjacentSquares.push(freshClick - 1)};
      if ((freshClick+1) % boardWidth !== 0) {adjacentSquares.push(freshClick + 1)};
